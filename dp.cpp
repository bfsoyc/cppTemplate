...
/*
高维前缀和：
一维数组 [2 5 4 1 2] 的前缀和数组 [2 7 11 12 14]
高维数组 A[i][j][k]..[l] 的前缀和计算，每次计算一个维度，固定其他维度进行累加。
我们感兴趣的是每个维度的长度为2时（取值为0，1）的情况，因为容易用一维数组实现。
如 A[0][1][0][1] 可以映射到 dp[5] （5 的二进制表示（0101））
计算第4个维度的前缀和（实际上只有一个需要计算）：
A[i][j][k][1] += A[i][j][k][0], (i,j,k)=(0,1,0)时，就是 dp[5] += dp[4]。
先计算哪个维度对结果是没有影响的。
*/
for (int i(1); i < ms; i <<= 1) // 从低位到高位枚举维度
	for (int s(0); s < ms; s++)
		if (!(s&i)) {	// 只有一半的状态满足该条件
			// TODO
		}